...
| 207 | From a drained, non-running state (`running = none` and `runtimeStatus ≠ .running`), every operation preserves `Drained`: an existing resource stays `released` and no new resource can appear (both allocation surfaces, `acquire` and `acquireActor`, are blocked) | PROVEN | `drained_step_drained`, `step_resources_none_run_none` | in_tree_model_proof | yes |
...
| 216 | Resources are owned by a **task** or an **actor** (`ResourceOwner` sum type); one ledger, one `Drained` predicate, one finalization discipline, and one evidence ledger cover both (RFC 091) | DOCUMENTED | `docs/resource-lifetime.md`, RFC 091, `ResourceOwner` | — | — |
| 217 | `acquireActor a` allocates a fresh actor-owned resource only when the runtime is running, the actor is open, and the actor exists (witnessed by a mailbox); it preserves all 33 `WellFormed` fields | PROVEN | `preserves_wf_acquireActor`, conformance `acquireActor_ok` / `acquireActor_invalid_closed_actor` / `acquireActor_invalid_missing_mailbox` | in_tree_model_proof | yes |
| 218 | Actor-owned resources survive individual task termination (`complete`/`cancel`/`fail` do not close them); they close only when the owning actor is closed, where `closeActor` marks the actor's `allocated` resources `closing` | PROVEN | `closeActor_marks_actor_resources_closing`, conformance `task_{complete,cancel,fail}_does_not_close_actor_resource`, `closeActor_marks_actor_resource_closing`, `finalize_actor_resource_released` | in_tree_model_proof | yes |
| 219 | `release t r` is invalid for an actor-owned resource (Tier 1: no manual actor release; the `release` guard requires owner `= .task t`) | PROVEN | conformance `release_task_on_actor_resource_invalid`, `release_non_owner_invalid` | in_tree_model_proof | yes |
| 220 | Unified `Drained` covers actor-owned resources: `stopWhenDrained` is blocked while an actor-owned resource is `allocated` or `closing`, and succeeds once it is finalized; `acquireActor` cannot re-leak after a drained stop (`runtimeStatus ≠ .running`) | PROVEN | conformance `stopWhenDrained_blocked_by_actor_allocated_resource` / `_blocked_by_actor_closing_resource` / `_succeeds_after_actor_resource_finalized`, `drained_step_drained` | in_tree_model_proof | yes |
| 221 | Under `Drained`, `closeActor` is inert on the resource ledger (no `allocated` resource remains to mark `closing`) — so the drain/frozen permanence spine is unaffected | PROVEN | `markActorResourcesClosing_eq_of_drained` | in_tree_model_proof | yes |
| 222 | Actor existence for acquisition is witnessed by a mailbox (`ActorExists`), not by `actorStatus` alone (total/default-active) — `acquireActor` for a never-created actor is rejected | PROVEN | conformance `acquireActor_invalid_missing_mailbox`, `ActorExists` guard | in_tree_model_proof | yes |
| 223 | `runtimeStatus = .stopped` is **not** a global guarantee of `Drained`: the two stop ops are intentionally distinct (`stopWhenIdle` = scheduler-quiescent; `stopWhenDrained` = quiescent + drained). No global `stopped → Drained` invariant is asserted (architect ruling, Option B) | DOCUMENTED | `docs/shutdown-semantics.md`, RFC 092 | — | — |
| 224 | A successful `stopWhenDrained` enters `CleanStopped` (`stopped ∧ Frozen`), which is drained and scheduler-quiescent; the clean-stop predicate is the contract handle for "clean shutdown" | PROVEN | `stopWhenDrained_enters_cleanStopped`, `cleanStopped_drained`, `cleanStopped_quiescent` | in_tree_model_proof | yes |
| 225 | From a clean stop, every subsequent operation sequence preserves `Frozen` (quiescent + drained); the exact `.stopped` label may relabel to `.shuttingDown` under `shutdown`, but never to `.running` | PROVEN | `cleanStopped_run_stays_frozen` | in_tree_model_proof | yes |
| 226 | `stopWhenIdle` **can** reach `.stopped` with a live (undrained) resource — a named witness that the two stop ops are intentionally different, so bare `.stopped` can never silently be claimed to mean clean | PROVEN | `stopWhenIdle_can_stop_undrained`, conformance `stopWhenIdle_stops_with_live_resource` | in_tree_model_proof | yes |
| 227 | `releaseActor a r` (control-plane, running-gated, symmetric with `acquireActor`) flips actor `a`'s own `allocated` resource to `released` and preserves all 33 `WellFormed` fields; it is invalid for a task-owned resource, the wrong actor, an already-released resource, or a non-running runtime | PROVEN | `preserves_wf_releaseActor`, conformance `releaseActor_ok` / `releaseActor_wrong_actor_invalid` / `releaseActor_on_task_resource_invalid` / `releaseActor_double_invalid` / `releaseActor_not_running_invalid` | in_tree_model_proof | yes |
| 228 | `release t r` (the task release) remains invalid for actor-owned resources — task and actor release paths are kept distinct (RFC 093 Tier 1 boundary) | PROVEN | conformance `release_task_on_actor_resource_invalid`, `releaseActor_on_task_resource_invalid` | in_tree_model_proof | yes |
| 229 | `releaseActor` provides a drain route for actor-owned resources that does not require `closeActor` + `finalize`: releasing a live actor resource lets a subsequent `stopWhenDrained` succeed | PROVEN | conformance `releaseActor_enables_drained_stop` | in_tree_model_proof | yes |
| 230 | RFC 062 Phase 1 proof-ergonomics: `mailbox_within_capacity` (RFC 056) is preserved by any step that leaves `mailboxPolicy` and `mailboxes` pointwise-stable; the three time blocks (`sleep`/`tick`/`wake`) now share this one field-specific helper instead of repeating five lines apiece, with the same theorem statements/names and unchanged axiom tiers | PROVEN | `wf_mailbox_capacity_pass`, `preserves_wf_sleep`, `preserves_wf_tick`, `preserves_wf_wake` | in_tree_model_proof | — |
| 231 | The prefix-defined public theorem surface (101 names: `preserves_wf_`/`step_preserves_`/`reachable_`/`bridge_`/`run_preserves_`) is snapshotted and diff-gated — a public theorem cannot be renamed or removed without a conscious migration record, catching renames the audit allowlist alone does not (RFC 062 §9.2) | DOCUMENTED | `scripts/public_theorem_index.py`, `docs/generated/public-theorems.md`, `docs/proof-api-stability.md`, RFC 062 | — | — |
| 232 | Proof-engineering policy is codified (auditability-first; no catch-all op classification; governed named simp-sets; no Phase 1–2 macros). The `henret_upd` named simp-set was evaluated and withdrawn — point-update lemmas do not compose under a named `simp only` set — so no simp-set ships in Phase 1; set adoption is deferred to the Phase-2 dense files (RFC 062 §13.4 applied at design time) | DOCUMENTED | `docs/proof-style.md`, RFC 062 | — | — |
| 233 | RFC 062 Phase 2A proof-ergonomics: the occurrence-identity fields under enqueue (`occ_fresh`/`occ_nodup`/`occ_disjoint`) are extracted into three `private` `*_under_enqueue` helpers shared by all five `send`/`inject` enqueue sites. The proof is occurrence-only (it never reads the envelope's `source`), so one helper trio covers both ops; `Messaging.lean` drops from 2078 to 1989 lines with the occ proof defined once instead of five times. Same theorem statements/names, unchanged axiom tiers (all six Messaging theorems remain `[propext, Quot.sound]`) | PROVEN | `wf_occ_fresh_under_enqueue`, `wf_occ_nodup_under_enqueue`, `wf_occ_disjoint_under_enqueue`, `preserves_wf_send`, `preserves_wf_inject` | in_tree_model_proof | — |
| 234 | `send`/`inject` mailbox-capacity reasoning is kept **explicit**, never disguised as a pass-through: because an enqueue grows a mailbox, each `mailbox_within_capacity` bullet proves the new length stays within capacity using the not-full guard (`Mailbox.enqueue_length` + `lt_capacity_of_not_full`), and is deliberately not folded into the occ helpers (architect Phase-2A §10) | PROVEN | `preserves_wf_send`, `preserves_wf_inject` (capacity bullets), `docs/proof-ergonomics-metrics.md` | in_tree_model_proof | — |
| 235 | RFC 062 Phase 2B-1 proof-ergonomics: five per-field pass-through helpers (`wf_waiters_owned_pass`, `wf_waiters_nodup_pass`, `wf_owned_has_mailbox_pass`, `wf_timer_nodup_pass`, `wf_timer_sorted_pass`) discharge the `WellFormed` fields a lifecycle op leaves pointwise-stable; adopted at 22 sites across `Lifecycle.lean` (`spawn`/`schedule`/`yield`/`complete`/`cancel`/`fail`/`spawnChild`), removing a defensive `by_cases` from `waiters_owned` in three blocks. `taskState`-reading fields (`waiters_waiting`/`timers_sleep`/`spawned_has_owner`/`owner_spawned`) left explicit (not pass-through). Same statements/names, unchanged axiom tiers, no macro/Shape-A/simp-set | PROVEN | `wf_waiters_owned_pass`, `wf_waiters_nodup_pass`, `wf_owned_has_mailbox_pass`, `wf_timer_nodup_pass`, `wf_timer_sorted_pass`, `preserves_wf_schedule`, `preserves_wf_yield`, `preserves_wf_complete` | in_tree_model_proof | — |
| 236 | RFC 069 Proof Dependency Budget: a generated, diff-gated per-theorem budget over the 156 audited theorems classifies each by constructiveness (constructive/classical/trusted = 122/33/1), import weight (core/bridge/standard/conformance/native), and public stability (public-stable/internal = 47/109), and lists every `Classical.choice` user explicitly. `scripts/proof_dependency_budget.py --check` is wired into gate 7 so a constructive→classical move (or a new audited theorem) cannot drift silently. Additive — no model/proof change, axioms unchanged | DOCUMENTED | `scripts/proof_dependency_budget.py`, `docs/generated/proof-dependency-budget.md`, `docs/proof-dependency-budget.md`, RFC 069 | — | — |
| 237 | RFC 094 mdBook documentation layout: the documentation surface is now an mdBook under `docs/src/` with `docs/book.toml`, a hand-maintained `docs/src/SUMMARY.md` (three reader paths), and `docs/src/introduction.md`. A documentation-integrity gate, `scripts/check_docs.sh` — separate from `scripts/check.sh` and never on the Lean proof path — runs `scripts/doc_summary_check.py` (every `docs/src` page reachable from SUMMARY, every SUMMARY target exists, every local Markdown link resolves) plus an mdBook build. Generated docs and all generator/gate read-write paths repointed to `docs/src/generated/` in one atomic move; the RFC-index link prefix was corrected so book-tree links resolve. No model/proof change, axioms unchanged, public theorem surface unchanged (101 names) | DOCUMENTED | `scripts/check_docs.sh`, `scripts/doc_summary_check.py`, `docs/book.toml`, `docs/src/SUMMARY.md`, RFC 094 | — | — |
