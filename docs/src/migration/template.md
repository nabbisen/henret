...
> **Convention (RFC 094 §9).** Migration guides may cite model counts (constructors, `WellFormed` fields) as they were at the *source* version. Always mark such numbers as historical (e.g. "was 29 at v0.33") so the `doc_count_check.py` stale-count gate — which excludes `docs/src/migration/` — and human readers do not mistake them for the current ground truth.
