...
| R6 | **Doc/claim drift.** Docs lagging grammar or theorem changes (historically recurrent). | Medium | Low–Medium | RFC 101 gates the mechanical part: both RFC indexes are generated from front matter (`extract_rfc_index.py --check`), repository-wide Markdown links are checked, and model counts (`RuntimeOp`/`TaskState`/`StepResult`/`WellFormed`) are derived from source by `doc_count_check.py`. **Not yet gated:** release-version strings, RFC-lifecycle counts, and milestone-status prose — the v0.34.6 review found roadmap drift that passed a green gate run. | Open — not release-blocking |
