...
[`docs/assurance-case.md`](assurance-case.md) §10). Risks are not
...
| R6 | **Doc/claim drift.** Docs lagging grammar or theorem changes (historically recurrent). | Medium | Low–Medium | Stale-phrase gate, doc-symbol checker, Semantic Impact Checklist, and the bridge-claim rule (RFC 051/052) — all enforced in `scripts/check.sh`. | Mitigated |
