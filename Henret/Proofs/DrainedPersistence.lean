...
/-- With no running task, no operation writes a resource at a slot that was
previously empty. `acquire` is the only operation that allocates a fresh
resource, and it requires a running task; every other operation either leaves
the ledger alone or only mutates existing (`some`) records. -/
theorem step_resources_none_run_none {s : RuntimeState} (h_run : s.running = none)
  ...
  | complete t => simp only [step]; (repeat' split) <;>
        first | rfl | exact hn | simp only [markClosingIf, hn]
  | cancel t => simp only [step]; (repeat' split) <;>
        first | rfl | exact hn | simp only [markClosingIf, hn]
  | fail t => simp only [step]; (repeat' split) <;>
        first | rfl | exact hn | simp only [markClosingIf, hn]
  | cancelTree root => simp only [step]; (repeat' split) <;>
        first | rfl | exact hn | simp only [applyCancelTree, markClosingIf, hn]
  | _ => simp only [step]; (repeat' split) <;> simp_all [upd]

/-- **Single-step drained persistence.** From a drained state with no running
task, every operation preserves `Drained`: an already-present resource is
`released` (`h_d`) and stays so under any op (`step_resources_eq_of_released`),
and no new resource can appear because `acquire` is blocked by `h_run`. -/
theorem drained_step_drained {s : RuntimeState} (h_wf : WellFormed s)
  ...
  | none =>
      rw [step_resources_none_run_none h_run hex op] at hr
...
runtime stopped while keeping `running = none` (the guard) and the ledger
unchanged. Hence the post-stop state meets the `drained_step_drained`
hypotheses, and the next operation cannot leak a resource. -/
theorem stopWhenDrained_then_step_drained {s : RuntimeState} (h_wf : WellFormed s)
  ...
  have h_wf' : WellFormed { s with runtimeStatus := .stopped } := by
    ...
    rwa [hstate] at hw
  rw [hstate]
  exact drained_step_drained h_wf' hguard.1 h_d op
